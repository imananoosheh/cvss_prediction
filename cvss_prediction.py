...
class NLPModelCNN(NLPModel):
	...
	def build_cnn_model(self):
		...
		train_encoding_padded = tf.keras.preprocessing.sequence.pad_sequences(train_encoding, maxlen = vectorizer._max_len, padding = "post")
		

		embedding_matrix = self.get_embedding_matrix(word_to_index_dict = vectorizer._w2i, vocab_len = vectorizer._vocan_len)
		y_train_one_hot = PreProcessing.get_one_hot_vectors(self.y_train)

		clf = DeepLearningModels().MultiChannelCNN(vectorizer._max_len, vectorizer._vocan_len, self.embedding_dim, embedding_matrix)
		clf.compile(loss = 'categorical_crossentropy', optimizer = 'adam', metrics = ['accuracy'])
		...
		if(self.X_test is not None):
			...
			test_encoding_paded = tf.keras.preprocessing.sequence.pad_sequences(test_encoding, maxlen = vectorizer._max_len, padding = "post")
...
class NLPDeploymentService:
	...
	def test_on_single_desc(self, vulnerability_description, trained_model_location, vectorizer_obj_file = None):
		...
		if(trained_model_location.split('.')[-1] == "pkl"):
			...
			test_encoding_paded = tf.keras.preprocessing.sequence.pad_sequences(clean_desc_vec, maxlen = vectorizer_obj._max_len, padding = "post")
...
class DeepLearningModels:
	...
	def CNN(self, max_len, vocan_len, embedding_dim, embedding_matrix):
		...
		embedding_layer = tf.keras.layers.Embedding(vocan_len, embedding_dim, weights = [embedding_matrix], input_length = max_len, trainable = False)(input_layer)
	...
	def MultiChannelCNN(self, max_len, vocan_len, embedding_dim, embedding_matrix):
		...
		embedding_layer_1 = tf.keras.layers.Embedding(vocan_len, embedding_dim, weights = [embedding_matrix], trainable = True)(input_layer)
		...
		embedding_layer_2 = tf.keras.layers.Embedding(vocan_len, embedding_dim, weights = [embedding_matrix], trainable = True)(input_layer)
...
if(__name__ == "__main__"):
	...
	nlp_model_object = NLPModel(X_train = df_2017_2018["description"].values, X_test = df_2019["description"].values, y_train = df_2017_2018["categorical_target"].values, y_test = df_2019["categorical_target"].values)
